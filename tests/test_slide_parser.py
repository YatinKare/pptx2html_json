...
def slide_parser():
    ...
    return SlideParser(
        slide_xml_path, slide_rels_path, pptx_unpacked_path, slide_width, slide_height
    )
...
def test_parse_slide_common_data(slide_parser):
    ...
    slide_xml_path_grad = os.path.abspath(
        "temp_pptx/ppt/slideLayouts/slideLayout133.xml"
    )
    slide_rels_path_grad = os.path.abspath(
        "temp_pptx/ppt/slideLayouts/_rels/slideLayout133.xml.rels"
    )
    parser_grad = SlideParser(
        slide_xml_path_grad,
        slide_rels_path_grad,
        os.path.abspath("temp_pptx"),
        slide_width=12192000,
        slide_height=6858000,
    )
    slide_obj_grad = parser_grad.parse_slide(
        slide_number=0
    )  # Slide number doesn't matter for layout

    assert (
        slide_obj_grad.common_slide_data.background_color is None
    )  # Background color is not directly set, but via gradFill
    assert slide_obj_grad.common_slide_data.background_gradient_fill is not None
    assert isinstance(
        slide_obj_grad.common_slide_data.background_gradient_fill, GradientFill
    )
    assert len(slide_obj_grad.common_slide_data.background_gradient_fill.stops) == 2
    assert (
        slide_obj_grad.common_slide_data.background_gradient_fill.stops[0].pos == 100000
    )
    assert (
        slide_obj_grad.common_slide_data.background_gradient_fill.stops[0].scheme_color
        == "accent4"
    )
    assert slide_obj_grad.common_slide_data.background_gradient_fill.stops[1].pos == 0
    assert (
        slide_obj_grad.common_slide_data.background_gradient_fill.stops[1].scheme_color
        == "accent2"
    )
...
def test_parse_slide_shapes_and_media(slide_parser):
    ...
    text_shape = next(
        (
            s
            for s in slide_obj.shapes
            if s.type == "shape"
            and s.text_frame
            and s.text_frame.paragraphs
            and "Agenda" in s.text_frame.paragraphs[0].text_runs[0].text
        ),
        None,
    )
    ...
    assert (
        agenda_run.properties.scheme_color is None
    )  # Assuming no explicit scheme color for Agenda
    assert (
        agenda_run.properties.font_face is None
    )  # Assuming no explicit font face for Agenda
    assert (
        agenda_run.properties.underline == False
    )  # Assuming no explicit underline for Agenda

    # Test for a picture (p:pic)
    picture_obj = next(
        (p for p in slide_obj.pictures if p.path.endswith("/ppt/media/image1.png")),
        None,
    )
