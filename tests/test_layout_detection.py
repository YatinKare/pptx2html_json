...
class TestLayoutDetection:
    ...
    def test_semantic_layout_names(self):
        ...
        semantic_layouts = [
            'title-only', 'title-and-content', 'title-and-bullets',
            'title-and-image', 'image-left', 'image-right', 'side-by-side',
            'two-column-text', 'quote-slide', 'image-gallery', 'custom'
        ]
        ...
        for slide in json_data.get("slides", []):
            ...
            assert layout not in powerpoint_layouts, f"Layout '{layout}' is PowerPoint internal name"
    ...
    def test_layout_consistency(self):
        ...
        for slide in json_data.get("slides", []):
            ...
            bullet_list_count = sum(1 for el in elements if el.get("type") == "bullet-list")
            image_count = sum(1 for el in elements if el.get("type") == "image")
            
            # Validate layout consistency with content
            if layout == "title-only":
                assert title_count == 1, f"title-only layout should have exactly 1 title"
                ...
                assert title_count >= 1, f"title-and-bullets layout should have at least 1 title"
                assert bullet_list_count >= 1, f"title-and-bullets layout should have at least 1 bullet list"
            
            elif layout == "title-and-image":
                assert title_count >= 1, f"title-and-image layout should have at least 1 title"
                assert image_count >= 1, f"title-and-image layout should have at least 1 image"
            
            elif layout == "side-by-side":
                assert len(elements) >= 2, f"side-by-side layout should have at least 2 elements"
