...
class TestElementClassification:
    ...
    def test_title_element_detection(self):
        ...
        for slide in slides:
            title_elements = [el for el in slide.get("elements", []) if el.get("type") == "title"]
            ...
            if title_elements:
                ...
                for title_el in title_elements:
                    ...
                    assert isinstance(title_el["text"], str), "Title text should be string"
                    assert len(title_el["text"].strip()) > 0, "Title should not be empty"
    ...
    def test_text_box_vs_bullet_list_classification(self):
        ...
        for bullet_list in bullet_lists:
            ...
            assert isinstance(bullet_list["items"], list), "Bullet list items should be array"
            assert len(bullet_list["items"]) > 0, "Bullet list should have at least one item"
            
            for item in bullet_list["items"]:
                assert "text" in item, "Bullet item should have text"
                assert isinstance(item["text"], str), "Bullet item text should be string"
            ...
            for element in slide.get("elements", []):
                ...
                if element_type == "title":
                    ...
                    for item in items:
                        ...
                        assert len(item_text.strip()) > 0, "Bullet item should not be empty"
                
                elif element_type == "image":
                    src = element.get("src", "")
                    assert len(src.strip()) > 0, "Image should have src"
                    assert not src.startswith("http"), "Should use local paths, not URLs"
