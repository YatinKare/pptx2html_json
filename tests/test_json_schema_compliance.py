...
class TestJsonSchemaCompliance:
    ...
    def test_slide_object_structure(self):
        ...
        for slide in slides:
            ...
            assert slide["id"].startswith('slide-'), "Slide ID should start with 'slide-'"
    ...
    def test_element_types(self):
        ...
        # Test element types
        valid_types = ['title', 'text-box', 'bullet-list', 'image', 'quote', 'chart-placeholder']
        
        for slide in json_data.get("slides", []):
            for element in slide.get("elements", []):
                assert "type" in element, "Element should have a type"
                assert element["type"] in valid_types, f"Invalid element type: {element['type']}"
        ...
        def check_no_positioning(obj, path=""):
            ...
            if isinstance(obj, dict):
                ...
                for prop in forbidden_props:
                    assert prop not in obj, f"Found forbidden positioning property '{prop}' at {path}"
