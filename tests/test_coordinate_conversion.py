...
class TestCoordinateConverter:
    ...
    def test_emu_to_px_conversion(self):
        ...
        assert (
            CoordinateConverter.emu_to_px(3080335) == 323
        )  # Title height (correct rounding)
    ...
    def test_extract_position_from_transform(self):
        ...
        transform = Transform(
            x=1280159, y=357809, cx=7983110, cy=3080335, rot=0, flipH=False, flipV=False
        )
    ...
    def test_generate_absolute_css(self):
        ...
        position = {"x": 134, "y": 38, "width": 838, "height": 324}
    ...
    def test_check_overflow_functionality(self):
        """Test overflow detection functionality."""
        position = {"x": 1200, "y": 600, "width": 200, "height": 200}

        overflow = CoordinateConverter.check_overflow(position)
    ...
    def test_prevent_overflow_functionality(self):
        """Test overflow prevention functionality."""
        position = {"x": 1200, "y": 600, "width": 200, "height": 200}

        adjusted = CoordinateConverter.prevent_overflow(position)

        # Should be constrained and aspect ratio preserved
        assert adjusted["x"] == 1200
        assert adjusted["y"] == 600
        assert adjusted["width"] == 80  # 1280 - 1200
        assert adjusted["height"] == 80  # Aspect ratio preserved (1:1)

    def test_generate_absolute_css_with_overflow_prevention(self):
        """Test CSS generation with overflow prevention."""
        position = {"x": 1200, "y": 600, "width": 200, "height": 200}

        css = CoordinateConverter.generate_absolute_css(
            position, z_index=3, prevent_overflow=True
        )

        assert "position: absolute;" in css
        assert "left: 1200px;" in css
        assert "top: 600px;" in css
        assert "width: 80px;" in css  # Adjusted
        assert "height: 80px;" in css  # Aspect ratio preserved
