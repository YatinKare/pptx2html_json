...
class TestBulletListDetection:
    ...
    def test_agenda_items_as_bullet_list(self):
        ...
        for slide in slides:
            for element in slide.get("elements", []):
                if element.get("type") == "title" and "Agenda" in element.get("text", ""):
                    agenda_slide = slide
                    break
        ...
        bullet_lists = [el for el in agenda_slide.get("elements", []) if el.get("type") == "bullet-list"]
        ...
        assert len(agenda_bullet_list["items"]) >= 3, "Should have multiple agenda items"
        
        # Check for topic items
        topic_items = [item for item in agenda_bullet_list["items"] if "Topic" in item.get("text", "")]
    ...
    def test_explicit_bullet_detection(self):
        ...
        for slide in json_data.get("slides", []):
            bullet_lists = [el for el in slide.get("elements", []) if el.get("type") == "bullet-list"]
            total_bullet_lists += len(bullet_lists)
        
        # Should find some bullet lists in the presentation
        assert total_bullet_lists > 0, "Should detect some bullet lists in the presentation"
    ...
    def test_bullet_list_item_structure(self):
        ...
        for slide in json_data.get("slides", []):
            for element in slide.get("elements", []):
                if element.get("type") == "bullet-list":
                    ...
                    assert "items" in element, "Bullet list should have 'items' property"
                    assert isinstance(element["items"], list), "Bullet list items should be an array"
                    
                    # Check structure of individual items
                    for item in element["items"]:
                        assert isinstance(item, dict), "Bullet list item should be an object"
                        assert "text" in item, "Bullet list item should have 'text' property"
                        assert isinstance(item["text"], str), "Bullet list item text should be a string"
        ...
        assert bullet_lists_found > 0, "Should find at least one bullet list to validate"
    ...
    def test_heuristic_bullet_detection(self):
        ...
        for slide in slides_with_bullets:
            ...
            for bullet_list in bullet_lists:
                ...
                assert len(items) >= 1, "Heuristically detected bullet lists should have at least one item"
                
                # Check that items have reasonable length (not too long for bullets)
                for item in items:
                    text = item.get("text", "")
                    assert len(text) < 200, f"Bullet item text too long: {len(text)} chars"
