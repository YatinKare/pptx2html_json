        ...
        for scheme_color, expected_rgb in test_cases:
            ...
            with Image.open(output_path) as img:
                ...
                assert pixel == expected_rgb, (
                    f"Failed for {scheme_color}: got {pixel}, expected {expected_rgb}"
                )
    ...
    def test_theme_color_fallback_to_defaults(self):
        ...
        for scheme_color, expected_rgb in gradient_test_cases:
            ...
            output_path = os.path.join(
                self.temp_dir, f"default_{scheme_color}_gradient.png"
            )
            ...
            with Image.open(output_path) as img:
                pixel = img.getpixel((50, 50))
                assert pixel == expected_rgb, (
                    f"Default color failed for {scheme_color} in gradient"
                )
