...
class BackgroundRenderer:
    ...
    def _render_image_background(
        self, canvas_image: Image.Image, image_path: str
    ) -> bool:
        ...
        try:
            ...
            if new_width < self.slide_width or new_height < self.slide_height:
                ...
                canvas_image.paste(
                    Image.new("RGB", (self.slide_width, self.slide_height), "white"),
                    (0, 0),
                )
    ...
    def _render_gradient_background(
        ...
        start_color = self._resolve_color(
            start_stop.color, start_stop.scheme_color, theme_colors
        )
        end_color = self._resolve_color(
            end_stop.color, end_stop.scheme_color, theme_colors
        )
    ...
    def _fill_solid_color(self, draw: ImageDraw.ImageDraw, hex_color: str):
        ...
        if len(hex_color) == 6:
            color = f"#{hex_color}"
            draw.rectangle([(0, 0), (self.slide_width, self.slide_height)], fill=color)
    ...
    def _resolve_color(
        self, color: str | None, scheme_color: str | None, theme_colors: dict | None
    ) -> str | None:
