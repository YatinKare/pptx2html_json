...
class StyleResolver:
    def __init__(
        self, slide_master: SlideMaster, presentation_defaults: PresentationDefaults
    ):
        """
    ...
    def resolve_paragraph_properties(
        ...
        """
        Resolve paragraph properties following the correct OOXML inheritance hierarchy.

        The function performs a search in the following strict order:
        1. Paragraph Properties: Check paragraph's <a:pPr> element first
        2. Layout List Styles: Check slide_layout.list_styles for the paragraph's level
        3. Master List Styles: Check slide_master.list_styles for the paragraph's level
        4. Presentation Defaults: Check presentation_defaults
        5. No Bullet: If nothing found, bullet_type remains None
        ...
        if p_element is not None:
            ...
            if p_pr_element is not None:
                ...
                bu_auto_num_element = p_pr_element.find(
                    ".//a:buAutoNum", namespaces=nsmap
                )
        ...
        # If no explicit bullet property was found on the paragraph,
        # check if it's a type of placeholder that shouldn't get list styles.
        if ph_type in ["title", "ctrTitle", "subTitle"]:
            # Title placeholders are not lists unless explicitly formatted.
            # Since we found no explicit formatting above, it has no bullet.
            return ParagraphProperties(bullet_type=None)

        # If it's a body or other content placeholder, THEN proceed with inheritance.
        # Use a lookup_level to handle paragraphs with a missing lvl attribute.
        lookup_level = p_level if p_level is not None else 0

        # Now, perform the inheritance search using lookup_level.
        # 1. Check Layout List Styles
        if (
            slide_layout
            and slide_layout.list_styles
            and lookup_level in slide_layout.list_styles
        ):
            layout_style = slide_layout.list_styles[lookup_level]
            if layout_style and layout_style.bullet_type:
                return self._merge_paragraph_properties(layout_style)

        # 2. Check Master List Styles
        if slide_layout and slide_layout.slide_master:
            if (
                slide_layout.slide_master.list_styles
                and lookup_level in slide_layout.slide_master.list_styles
            ):
                master_style = slide_layout.slide_master.list_styles[lookup_level]
                if master_style and master_style.bullet_type:
                    return self._merge_paragraph_properties(master_style)

        # 3. Check Presentation Defaults
        if (
            self.presentation_defaults
            and hasattr(self.presentation_defaults, "list_styles")
            and self.presentation_defaults.list_styles
            and lookup_level in self.presentation_defaults.list_styles
        ):
            default_style = self.presentation_defaults.list_styles[lookup_level]
            if default_style and default_style.bullet_type:
                return self._merge_paragraph_properties(default_style)

        # If still nothing is found, it's not a bullet.
        return ParagraphProperties(bullet_type=None)

    def _get_layout_style_by_ph_type(
        self, slide_layout: SlideLayout, ph_type: str | None
    ...
    def _get_master_style_by_ph_type(
        self, ph_type: str | None
    ) -> ParagraphProperties | None:
    ...
    def _check_layout_list_style(
        self, slide_layout: SlideLayout, p_level: int | None
    ) -> ParagraphProperties | None:
        """
    ...
    def _check_master_text_style(
        self, slide_master: SlideMaster | None, ph_type: str | None, p_level: int | None
    ) -> ParagraphProperties | None:
        """
    ...
    def _check_presentation_defaults(
        self, p_level: int | None
    ) -> ParagraphProperties | None:
        """
        ...
        """
        if (
            self.presentation_defaults.default_paragraph_properties
            and self.presentation_defaults.default_paragraph_properties.bullet_type
            is not None
        ):
            return self._merge_paragraph_properties(
                self.presentation_defaults.default_paragraph_properties
            )

        return None

    def _extract_bullet_properties_from_element(
        self, element
    ) -> ParagraphProperties | None:
        """
    ...
    def _merge_paragraph_properties(
        self, base_props: ParagraphProperties
    ) -> ParagraphProperties:
        """
    ...
    def resolve_run_properties(
        ...
        if (
            hasattr(self.presentation_defaults, "default_run_properties")
            and self.presentation_defaults.default_run_properties
            and self.presentation_defaults.default_run_properties.font_size
        ):
            resolved_run_props.font_size = (
        ...
        if slide_layout:
            ...
            layout_style = self._get_layout_style_by_ph_type(
                slide_layout, None
            )  # We don't have ph_type here
            if (
                layout_style
                and layout_style.default_run_properties
                and layout_style.default_run_properties.font_size
            ):
                resolved_run_props.font_size = (
                    layout_style.default_run_properties.font_size
                )

            # Then check master styles through the layout
            if slide_layout.slide_master:
                master_style = self._get_master_style_by_ph_type(
                    None
                )  # We don't have ph_type here
                if (
                    master_style
                    and master_style.default_run_properties
                    and master_style.default_run_properties.font_size
                ):
                    resolved_run_props.font_size = (
                        master_style.default_run_properties.font_size
                    )
        ...
        if slide_layout and slide_layout.list_styles and list_level is not None:
            ...
            if (
                layout_level_style
                and layout_level_style.default_run_properties
                and layout_level_style.default_run_properties.font_size
            ):
                resolved_run_props.font_size = (
        ...
        if (
            paragraph_properties.default_run_properties
            and paragraph_properties.default_run_properties.font_size
        ):
            resolved_run_props.font_size = (
        ...
        if run_element is not None:
            ...
            if run_element.get("b") is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.bold is not None
            ):
                resolved_run_props.bold = (
                    paragraph_properties.default_run_properties.bold
                )
            ...
            if run_element.get("i") is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.italic is not None
            ):
                resolved_run_props.italic = (
                    paragraph_properties.default_run_properties.italic
                )
            ...
            if run_element.get("u") is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.underline is not None
            ):
                resolved_run_props.underline = (
                    paragraph_properties.default_run_properties.underline
                )
            ...
            if run_element.get("cap") is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.cap is not None
            ):
            ...
            if latin_font_element is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.font_face is not None
            ):
                resolved_run_props.font_face = (
                    paragraph_properties.default_run_properties.font_face
                )
            ...
            if font_ref_element is not None:
                ...
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.font_ref is not None
            ):
                resolved_run_props.font_ref = (
                    paragraph_properties.default_run_properties.font_ref
                )
            ...
            if solid_fill_element is not None:
                ...
                if srgb_color_element is not None:
                    ...
                    if scheme_color_element is not None:
                        resolved_run_props.scheme_color = scheme_color_element.get(
                            "val"
                        )
            elif (
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.color is not None
            ):
                resolved_run_props.color = (
                    paragraph_properties.default_run_properties.color
                )
            elif (
                paragraph_properties.default_run_properties
                and paragraph_properties.default_run_properties.scheme_color is not None
            ):
                resolved_run_props.scheme_color = (
                    paragraph_properties.default_run_properties.scheme_color
                )
