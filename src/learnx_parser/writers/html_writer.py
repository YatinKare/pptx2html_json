...
class HtmlWriter:
    ...
    def _get_image_crop_css(self, blip_fill: BlipFill):
        ...
        if blip_fill and (
            blip_fill.src_rect_t is not None
            or blip_fill.src_rect_b is not None
            or blip_fill.src_rect_l is not None
            or blip_fill.src_rect_r is not None
        ):
            # Convert 1000ths of a percentage to CSS percentage string
            top = (
                f"{blip_fill.src_rect_t / 1000:.2f}%"
                if blip_fill.src_rect_t is not None
                else "0%"
            )
            bottom = (
                f"{blip_fill.src_rect_b / 1000:.2f}%"
                if blip_fill.src_rect_b is not None
                else "0%"
            )
            left = (
                f"{blip_fill.src_rect_l / 1000:.2f}%"
                if blip_fill.src_rect_l is not None
                else "0%"
            )
            right = (
                f"{blip_fill.src_rect_r / 1000:.2f}%"
                if blip_fill.src_rect_r is not None
                else "0%"
            )
    ...
    def _get_paragraph_style_css(self, paragraph: Paragraph) -> str:
        ...
        if paragraph.properties and paragraph.properties.indent is not None:
            paragraph_style += (
                f"padding-left: {self._emu_to_px(paragraph.properties.indent)}px;"
            )
    ...
    def _get_run_style_css(self, run: TextRun) -> str:
        ...
        if run.properties:
            ...
            if run.properties.font_size:
                run_style += (
                    f"font-size: {self._emu_to_px(run.properties.font_size)}px;"
                )
            ...
            # Apply scheme color if specified (using CSS variables for theming)
                ...
                run_style += (
                    f"color: var(--{run.properties.scheme_color}-color, black);"
                )
    ...
    def _render_graphic_frame_html(
        self, element: GraphicFrame, parent_x: int = 0, parent_y: int = 0
    ) -> str:
...
"""
    ...
    def _render_group_shape_html(
        ...
        for child in element.children[0]:  # shapes
            children_html += self._render_shape_html(
                child, element.transform.x, element.transform.y
            )
        for child in element.children[1]:  # pictures
            children_html += self._render_picture_html(
                child, element.transform.x, element.transform.y
            )
        for child in element.children[2]:  # group_shapes
            children_html += self._render_group_shape_html(
                child, element.transform.x, element.transform.y
            )
        for child in element.children[3]:  # graphic_frames
            children_html += self._render_graphic_frame_html(
                child, element.transform.x, element.transform.y
            )
    ...
    def _render_shape_html(
        ...
        if element.text_frame:
            for paragraph in element.text_frame.paragraphs:
                ...
                text_content_html += (
                    f'<p style="{paragraph_style}">{text_runs_html}</p>'
                )
...
"""
    ...
    def _render_slide_content(self, slide: Slide) -> str:
        content_html = ""
        slide_elements_by_id = {
            el.id: el
            for el in slide.shapes
            + slide.pictures
            + slide.group_shapes
            + slide.graphic_frames
        ...
        if slide.slide_layout:
            for layout_ph in slide.slide_layout.placeholders:
                for element_id, element in slide_elements_by_id.items():
                    if (
                        hasattr(element, "ph_type")
                        and element.ph_type == layout_ph.ph_type
                        and (
                            layout_ph.ph_idx is None
                            or (
                                hasattr(element, "ph_idx")
                                and element.ph_idx == layout_ph.ph_idx
                            )
                        )
                    ):
        ...
        all_slide_elements = (
            slide.shapes + slide.pictures + slide.group_shapes + slide.graphic_frames
        )
    ...
    def _render_picTx_layout(
        self, slide: Slide, placeholder_elements: dict
    ) -> tuple[str, set]:
        content_html = ""
        used_element_ids = set()
...
"""
    ...
    def _render_obj_layout(
        self, slide: Slide, placeholder_elements: dict
    ) -> tuple[str, set]:
        return self._render_tx_layout(slide, placeholder_elements)  # Same as tx

    def _render_titleOnly_layout(
        self, slide: Slide, placeholder_elements: dict
    ) -> tuple[str, set]:
        content_html = ""
        used_element_ids = set()

        title_element = placeholder_elements.get("title") or placeholder_elements.get(
            "ctrTitle"
        )
...
"""
                if isinstance(element, Shape):
                    content_html += self._render_shape_html(
                        element, use_absolute_pos=False
                    )
                elif isinstance(element, Picture):
                    content_html += self._render_picture_html(
                        element, use_absolute_pos=False
                    )
                elif isinstance(element, GroupShape):
                    content_html += self._render_group_shape_html(
                        element, use_absolute_pos=False
                    )
                elif isinstance(element, GraphicFrame):
                    content_html += self._render_graphic_frame_html(
                        element, use_absolute_pos=False
                    )
                content_html += """
    </div>
"""
    ...
    def write_slide_html(self, slide: Slide, slide_number: int):
        ...
        layout_class = (
            f" {slide.slide_layout.type}-layout"
