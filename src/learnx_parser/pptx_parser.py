...
class PptxParser:
    ...
    def parse_presentation(self):
        ...
        for i, r_id in enumerate(slide_r_ids):
            ...
            slide_parser = SlideParser(slide_xml_path, slide_rels_path)
            slide_data = slide_parser.parse_slide()

            # Use 1-based indexing for slide number in output
            self.html_writer.write_slide_html(slide_data, i + 1)
            self.json_writer.write_slide_json(slide_data, i + 1)
